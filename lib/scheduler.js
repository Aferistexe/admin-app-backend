...
// Получает список steam64 всех старших из публичного API.
// Использует публичный эндпоинт (без auth) — cron работает вне запроса.
...
async function runSnapshot(when) {
  const ids = await getSeniorSteamIds();
  if (ids.length === 0) {
    console.log(`[scheduler] ${when}: нет старших для снимка`);
    return;
  }
  console.log(`[scheduler] ${when}: снимок для ${ids.length} старших...`);
  const results = await batchSnapshot(ids, when, null, 5);
