...
// steamLoginSecure из DevTools обычно URL-кодирован (например %7C%7C вместо ||).
// Декодируем decodeURIComponent — Steam требует раскодированное значение.
const cleanCookieValue = (raw) => {
  let v = (raw || '').trim();
  if (!v) return '';
  try {
    const dec = decodeURIComponent(v);
    // декодируем, только если значение изменилось (значит реально было кодировано)
    v = dec;
  } catch {
    // decodeURIComponent может бросить на некорректной последовательности —
...
const getSteamCookie = () => {
  const parts = [];
  const sessionid = cleanCookieValue(process.env.STEAM_SESSIONID);
  const loginSecure = cleanCookieValue(process.env.STEAM_LOGIN_SECURE);
 ...
 *
 * ВАЖНО про Node.js fetch (undici): при redirect:'manual' заголовок `location`
 * НЕ доступен через res.headers — это отличается от браузерного fetch. Поэтому
 * идём по редиректам сами, читая `location` из ответа (undici отдаёт его для 3xx),
 * и передаём Cookie на каждый прыжок (у fetch нет cookie-jar).
 *
...
const completeSteamOpenId = async (openidUrl) => {
  ...
  // OpenID-флоу Steam: при валидной сессии Steam возвращает 200 со страницей,
  // содержащей АВТО-САБМИТ-форму (POST на callback с openid-параметрами).
  // Поэтому просто следовать 302 недостаточно — нужно найти и отправить эту форму.
  // Цепочка: GET openid-ссылки → (возможно 302) → 200 страница с формой → POST формы
  // → odrp4.ru/api/steam/callback (200, HTML с tempToken).

  let currentUrl = openidUrl;
  for (let i = 0; i < 15; i++) {
    const res = await fetchJson(currentUrl, { method: 'GET', redirect: 'manual', headers });
    const status = res.status;
    const location = res.headers.get('location');

    // Попали на callback odrp4 — tempToken в теле этой страницы.
    if (currentUrl.includes('/api/steam/callback')) {
      const html = await res.text().catch(() => '');
      const tt = parseTempToken(html);
      if (tt) return tt;
      throw new Error(
        `callback без tempToken (подпись OpenID не подтверждена или аккаунт не одобрен на odrp4). ` +
        `Фрагмент ответа: ${html.slice(0, 160)}`
      );
    }

    // 3xx — следуем по редиректу с тем же cookie.
    if (location) {
    ...
    // 200 — это страница Steam. Ищем авто-сабмит-форму OpenID (POST на callback).
    const html = await res.text().catch(() => '');

    // tempToken могут отдать и напрямую (маловероятно, но проверим).
    const ttDirect = parseTempToken(html);
    if (ttDirect) return ttDirect;

    // Steam OpenID возвращает <form action="https://odrp4.ru/api/steam/callback" method="post">
    // со скрытыми инпутами openid.* — отправляем её POST'ом.
    const formInfo = extractOpenIdForm(html, currentUrl);
    if (formInfo) {
      // eslint-disable-next-line no-await-in-loop
      const postRes = await fetchJson(formInfo.action, {
        method: 'POST',
        redirect: 'manual',
        headers: {
          ...headers,
      ...
      // После POST попадаем на callback odrp4.
      if (formInfo.action.includes('/api/steam/callback')) {
        const cbHtml = await postRes.text().catch(() => '');
        const tt = parseTempToken(cbHtml);
        if (tt) return tt;
        throw new Error(
          `callback (после POST) без tempToken. status=${postRes.status}. Фрагмент: ${cbHtml.slice(0, 160)}`
        );
      }
      // Иначе продолжаем цикл по новому location/странице.
      const postLoc = postRes.headers.get('location');
      if (postLoc) {
        currentUrl = postLoc.startsWith('http') ? postLoc : new URL(postLoc, formInfo.action).href;
        continue;
      }
      break;
    }

    // Нет формы и нет редиректа — Steam не признал сессию (страница входа/выбора).
    if (currentUrl.includes(STEAM_OPENID_HOST)) {
      throw new Error(
        `Steam не признал сессию (steamLoginSecure невалиден/истёк или не задан sessionid). ` +
        `status=${status}. Возможно нужен STEAM_SESSIONID. Фрагмент: ${html.slice(0, 160)}`
      );
    }
    break;
  }
...
/**
 * Извлекает авто-сабмит-форму OpenID из HTML Steam.
 * Steam рендерит <form id="...LoginForm" action="<callback>" method="post">
 * со скрытыми <input name="openid.*" value="...">. Возвращает { action, body }.
 * @returns {{action:string, body:string}|null}
 */
const extractOpenIdForm = (html, baseUrl) => {
  if (!html) return null;
  // Находим форму с openid-инпутами. action может быть в одинарных/двойных кавычках.
  const formMatch = html.match(/<form[^>]*action=["']([^"']+)["'][^>]*method=["']post["'][^>]*>([\s\S]*?)<\/form>/i)
    || html.match(/<form[^>]*method=["']post["'][^>]*action=["']([^"']+)["'][^>]*>([\s\S]*?)<\/form>/i);
  if (!formMatch) return null;

  const rawAction = formMatch[1];
  const inner = formMatch[2];

  // action обычно абсолютный (https://odrp4.ru/api/steam/callback).
  const action = rawAction.startsWith('http')
    ? rawAction
    : new URL(rawAction, baseUrl).href;

  // Собираем скрытые инпуты → application/x-www-form-urlencoded body.
  ...
  // Запасной вариант: value перед name (другой порядок атрибутов).
  const inputRe2 = /<input[^>]*value=["']([^"']*)["'][^>]*name=["']([^"']+)["'][^>]*>/gi;
  while ((m = inputRe2.exec(inner)) !== null) {
    params.append(m[2], m[1]);
    found = true;
  }
 ...
 * Помощников (log.helpers) НЕ учитываем (иначе организатор-помощник получает
 * лишние фасты). Раньше считали и то и то → Архитектор: 6 + 1 helper = 7 (неверно).
 *
