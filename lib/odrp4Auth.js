...
const completeSteamOpenId = async (openidUrl) => {
  ...
  // Идём вручную, потому что нужен cookie на каждом шаге и контроль callback-страницы.
  ...
  for (let i = 0; i < 15; i++) {
    ...
    if (currentUrl.includes('/api/steam/callback')) {
      ...
      const tt = parseTempToken(lastBody);
      if (tt) return tt;
      // tempToken не распарсился — значит на callback пришли с невалидной подписью
      // или Steam не подтвердил аккаунт.
      throw new Error('callback без tempToken (подпись OpenID не подтверждена или аккаунт не одобрен на odrp4)');
    }

    const location = res.headers.get('location');

    // Редирект — следуем по нему с тем же cookie.
    if (location) {
    ...
    // Нет редиректа — это финальная страница. Пробуем достать tempToken
    // (могут отдать напрямую) или определить, что Steam не пустил.
    lastBody = await res.text().catch(() => '');
    const tt = parseTempToken(lastBody);
    if (tt) return tt;

    if (currentUrl.includes(STEAM_OPENID_HOST)) {
      // Steam вернул страницу входа/выбора — сессия не признана.
      throw new Error('Steam не признал сессию (steamLoginSecure невалиден или истёк)');
    }
    break;
  }

  // Последняя попытка — вдруг tempToken есть в финальном теле.
  const tt = parseTempToken(lastBody);
  if (tt) return tt;
