...
// полный снимок недели. extraBy — { [steam64]: { fasts?, shifts?, reports? } }
// переопределяет/дополняет метрики из БД (например, реальные фасты из odrp4).
// limit — размер чанка конкурентности.
// Возвращает массив результатов [{ steam64, ok, hours, error }].
// ------------------------------------
async function batchSnapshot(items, when, userId = null, limit = 5, extraBy = {}) {
  const db = getDB();
    ...
    const settled = await Promise.all(
      chunk.map(async (id) => {
        try {
          ...
          // extraBy имеет приоритет над метриками из БД (реальные фасты из odrp4).
          const snap = when === 'end'
            ? { tickets: ticketsBy[id], ...(metricsBy[id] || {}), ...(extraBy[id] || {}) }
            : null;
