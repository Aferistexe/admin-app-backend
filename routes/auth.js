...
router.post('/login', loginLimiter, async (req, res) => {
  ...
  try {
    ...
    if (user.locked_until && new Date(user.locked_until) > new Date()) {
      ...
      return res.status(423).json({ error: `Аккаунт заблокирован. Попробуйте через ${minutesLeft} минут.` });
    }
    ...
    if (!isValidPassword) {
      ...
      await db.query('UPDATE users SET login_attempts = $1, locked_until = $2 WHERE id = $3', [newAttempts, lockedUntil, user.id]);

      await logAction(user.id, 'login_failed', 'user', user.id, { attempts: newAttempts, locked: !!lockedUntil }, ipAddress, userAgent);

      return res.status(401).json({ 
        error: lockedUntil ? `Неверный пароль. Аккаунт заблокирован на 15 минут.` : `Неверный пароль. Осталось попыток: ${5 - newAttempts}`
      });
    }

    await db.query('UPDATE users SET login_attempts = 0, locked_until = NULL, last_login = NOW() WHERE id = $1', [user.id]);
    ...
    await db.query(
      ...
      [user.id, refreshToken, new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), ipAddress, userAgent]
    );
...
router.post('/refresh', async (req, res) => {
  ...
  try {
    ...
    await db.query(
      `INSERT INTO refresh_tokens (user_id, token, expires_at, ip_address, user_agent) VALUES ($1, $2, $3, $4, $5)`,
      [user.id, newRefreshToken, new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), req.ip, req.get('User-Agent')]
    );
...
router.post('/logout', authenticateToken, async (req, res) => {
  ...
  try {
    await db.query('UPDATE refresh_tokens SET revoked = TRUE WHERE token = $1 AND user_id = $2', [refreshToken, req.user.id]);
...
router.post('/register', authenticateToken, checkRole(['admin']), async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: 'Пользователь создан', user: { username, name: name || username, role: newRole } });
  } catch (err) {
