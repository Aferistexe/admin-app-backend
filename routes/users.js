...
router.put('/:id/role', 
  ...
  async (req, res) => {
    ...
    try {
      const userResult = await db.query('SELECT id, username, name, role, email FROM users WHERE id = $1', [id]);
      ...
      res.json({ message: `Роль изменена на ${role}`, user: { id, username: user.username, role } });
    } catch (err) {
